...
const ChartSection: React.FC = () => {
  ...
  const data = useMemo(() => labels.map(() => Math.floor(Math.random() * 200) + 10), [labels]);
  const donutTotal = donutData.reduce((acc, cur) => acc + cur.value, 0);
  const data1 = dataMap[view1];

  return (
    <div className="w-full min-h-screen px-4 md:px-10 py-6 space-y-8">
      {/* Chart Row */}
      <div className="flex flex-col lg:flex-row gap-6 w-full">
        ...
        <div className="bg-white shadow-lg rounded-xl p-6 flex-1">
          <div className="flex justify-between items-center mb-4">
            ...
            <select
              ...
              className="border rounded-full px-3 py-1 text-sm bg-gray-100 hover:bg-gray-200"
            >
          ...
          <div className="flex items-end justify-center gap-2 overflow-x-auto scrollbar-hide">
            {labels.map((label, i) => (
        ...
        {/* Donut Chart */}
        <div className="bg-white shadow-lg rounded-xl p-6 flex-1 flex flex-col md:flex-row">
          <div className="relative w-full md:w-1/2">
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={donutData} innerRadius={60} outerRadius={80} paddingAngle={3} dataKey="value">
          ...
          <div className="w-full md:w-1/2 flex flex-col justify-center pl-4 space-y-3">
            {donutData.map((entry, index) => (
        ...
        {/* Map (Summary List) */}
        <div className="bg-white shadow-lg rounded-xl p-6 flex-1">
          <h2 className="text-lg font-semibold mb-2">Top Sales Locations</h2>
          <h1 className="text-3xl font-bold">{totalCustomers} M</h1>
          <p className="text-sm text-gray-500 mb-4">Our most customers in India</p>
            ...
            {Object.entries({
              ...
              <div key={label} className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                  <span className="w-4 h-4 rounded-full" style={{ backgroundColor: categoryColors[label] }} />
      ...
      <div className="bg-white shadow-lg rounded-xl p-6 w-full">
            ...
            {(["Week", "Month", "Year", "All"] as ViewType1[]).map((v) => (
              <button
                ...
                className={`px-4 py-1 rounded-full text-sm font-medium ${
                  view1 === v ? "bg-sky-400 text-white" : "bg-black text-white hover:bg-gray-800"
                }`}
              >
                {v}
              </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center mt-6 border-t pt-4 text-sm text-gray-600">
          <div>
              ...
              {activityData.map((item, idx) => (
                <tr key={idx} className="border-b">
                  ...
                  {item.names.map((name, i) => (
                    <td key={i} className="py-3 px-4">{name}</td>
                  ))}
